...
Renew's checkout uses **Stripe hosted Checkout**: the shopper clicks *Pay*, is
redirected to Stripe's secure page (cards, Apple/Google Pay, Link, 3-D Secure),
and returns to the site. The order is recorded only when payment succeeds, via
the Stripe webhook. Everything is env-gated — until the keys are set, checkout
falls back to TagadaPay (if configured) or pay-on-delivery.

## How it works

1. Confirm step → `POST /.netlify/functions/create-checkout-session`
   re-prices the cart from Supabase, applies the affiliate discount, and returns
   a Stripe-hosted URL. The browser redirects there.
2. Stripe processes the payment and redirects back to
   `/checkout?stripe=success&order=RN-…` (or `?stripe=cancel`).
3. Stripe calls `POST /.netlify/functions/stripe-webhook`
   (`checkout.session.completed`) → the paid order is recorded in Supabase and
   the owner + customer emails go out. Idempotent, so Stripe retries are safe.
...
### 3. Build env (turns the card button on — safe to expose)
```
